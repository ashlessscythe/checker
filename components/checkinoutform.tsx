...
export default function CheckInOutForm({ shouldFocus }: CheckInOutFormProps) {
  ...
  const triggerFlashEffect = useCallback(
    (type: "checkin" | "checkout", userName: string) => {
  ...
  const handleCheckInOut = useCallback(async () => {
    ...
    try {
      ...
      if (!lastPunch) {
        ...
        const RESET_HOURS = parseInt(
          process.env.NEXT_PUBLIC_RESET_HOURS || "14",
          10
        );
    ...
    <>
      ...
      {flashEffect.active && (
        <div
          className={`fixed inset-0 z-50 pointer-events-none ${
            flashEffect.type === "checkin" ? "flash-checkin" : "flash-checkout"
          }`}
          aria-hidden="true"
          role="status"
          aria-live="polite"
      ...
      {successMessage.type && successMessage.user && (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-40 pointer-events-none">
          <div
            className={`px-8 py-6 rounded-2xl shadow-2xl border-4 ${
          ...
          >
            <div className="text-center">
              ...
              <p className="text-lg opacity-90">{successMessage.user}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col items-center">
        <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md">
          ...
          {/* Status indicator */}
          <div
            className="mb-4 p-3 rounded-lg bg-gray-100 dark:bg-gray-700 border-l-4 border-blue-500"
            role="status"
            aria-live="polite"
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Swipe or place your badge near the scanner or type your ID
            </p>
          </div>
